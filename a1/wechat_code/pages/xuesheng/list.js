...
res = await page(`xuesheng`, searchForm);
} else {
res = await list(`xuesheng`, searchForm);
}
...
wx.navigateTo({
url: `/pages/xuesheng/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("xuesheng",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
wx.navigateTo({
url: `/pages/xuesheng/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/xuesheng/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 xuesheng Xuesheng xuesheng ${pathname}
console.log('Xuesheng  xuesheng')
isAuthObj['addAuth']   = utils.isAuth("xuesheng", "新增")
isAuthObj['delAuth']  = utils.isAuth("xuesheng", "删除")
isAuthObj['editAuth']   =utils.isAuth("xuesheng", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("xuesheng", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("xuesheng", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("xuesheng", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("xuesheng",obj)
    }else{
         resList= await list("xuesheng",obj)
    }
