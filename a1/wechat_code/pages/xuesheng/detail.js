...
goodname:  this.data.detailList.xuehao,
...
if (id) {
const {data} = await detail("xuesheng",id)
this.setData({
payAuth:utils.isAuthFront('xuesheng','支付')
})
...
this.setData({
title:  this.data.detailList.xuehao,
    });
...
const resUpdate = await update('xuesheng', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','xuesheng');
...
async reply() {
const detailList = this.data.detailList
const res = await update("xuesheng", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
