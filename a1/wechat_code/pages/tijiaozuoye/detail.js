...
zuoyepigaiisAuthFront:'',
isAuthzuoyepigai:'',
zhuangtaiCrossSelect:'已批改,待批改'.split(','),
},
async    onLoad(option) {
let authobj={}
authobj['pageIsAuth']= wx.getStorageSync('isAuth')
authobj['isAuthzuoyepigai']= utils.isAuth('tijiaozuoye','作业批改')
authobj['zuoyepigaiisAuthFront']=  utils.isAuthFront('tijiaozuoye','作业批改')
...
if (id) {
const {data} = await detail("tijiaozuoye",id)
this.setData({
payAuth:utils.isAuthFront('tijiaozuoye','支付')
})
...
data.tijiaozuoyeneirong=utils.addImgWidthStyle(data.tijiaozuoyeneirong,'100%')
...
const resUpdate = await update('tijiaozuoye', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','tijiaozuoye');
...
async reply() {
const detailList = this.data.detailList
const res = await update("tijiaozuoye", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','tijiaozuoye');
