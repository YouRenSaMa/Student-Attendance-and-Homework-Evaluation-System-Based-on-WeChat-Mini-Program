...
if(this.data.zuoyemingcheng){
    searchForm['zuoyemingcheng'] = '%' + this.data.zuoyemingcheng + '%'
}
...
res = await page(`tijiaozuoye`, searchForm);
} else {
res = await list(`tijiaozuoye`, searchForm);
}
...
wx.navigateTo({
url: `/pages/tijiaozuoye/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("tijiaozuoye",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
wx.navigateTo({
url: `/pages/tijiaozuoye/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/tijiaozuoye/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 tijiaozuoye Tijiaozuoye tijiaozuoye ${pathname}
console.log('Tijiaozuoye  tijiaozuoye')
isAuthObj['addAuth']   = utils.isAuth("tijiaozuoye", "新增")
isAuthObj['delAuth']  = utils.isAuth("tijiaozuoye", "删除")
isAuthObj['editAuth']   =utils.isAuth("tijiaozuoye", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("tijiaozuoye", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("tijiaozuoye", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("tijiaozuoye", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("tijiaozuoye",obj)
    }else{
         resList= await list("tijiaozuoye",obj)
    }
