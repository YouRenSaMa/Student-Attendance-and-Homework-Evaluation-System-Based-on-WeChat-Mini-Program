...
second: "",
timer: null,
tijiaozuoyeisAuthFront:'',
isAuthtijiaozuoye:'',
},
async    onLoad(option) {
let authobj={}
authobj['pageIsAuth']= wx.getStorageSync('isAuth')
authobj['isAuthtijiaozuoye']= utils.isAuth('kechengzuoye','提交作业')
authobj['tijiaozuoyeisAuthFront']=  utils.isAuthFront('kechengzuoye','提交作业')
...
goodname:  this.data.detailList.zuoyemingcheng,
...
if (id) {
const {data} = await detail("kechengzuoye",id)
this.setData({
payAuth:utils.isAuthFront('kechengzuoye','支付')
})



if (data?.reversetime) {
utils.countdown(data?.reversetime,
function (reveseTime) {
that.setData({
    reveseTime
})
...
let objClo={}
objClo['picture']= data.fengmian.split(',');
data.zuoyeneirong=utils.addImgWidthStyle(data.zuoyeneirong,'100%')
...
this.setData({
title:  this.data.detailList.zuoyemingcheng,
    });
...
const resUpdate = await update('kechengzuoye', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','kechengzuoye');
...
async reply() {
const detailList = this.data.detailList
const res = await update("kechengzuoye", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','kechengzuoye');
