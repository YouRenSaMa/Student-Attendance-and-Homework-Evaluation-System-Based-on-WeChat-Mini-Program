...
ro.zuoyeneirong = true;
continue;
}else{
}

if(o=='reversetime'){
    refobjempty["reversetime"]=obj[o]
ro.reversetime = true;
continue;
}else{
}
...
detailList,
 ...
 reversetime:utils.getCurrentDate("yMDhms"),
}
this.data.editStatus? getApp().globalData.editorContent=data.zuoyeneirong:""
...
if(this.data.reversetime?.includes("请选择") || this.data.reversetime==""){
wx.showToast({
icon: "none",
title: `截止提交时间不能为空`,
})
...
zuoyeneirong: getApp().globalData.editorContent,
reversetime: this.data. reversetime,
}
...
let zuoyeneirong=  getApp().globalData.editorContent
ruleForm['zuoyeneirong']=getApp().globalData.editorContent
if(zuoyeneirong==='' ){
wx.showToast({
icon: "none",
title: `作业内容不能为空`,
...
let corssRes = await list(`kechengzuoye`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
//跨表计算
if (ruleForm.id ) {
await update(`kechengzuoye`, ruleForm)
}
else {
 this.data?.commentScore?ruleForm['score']= this.data.commentScore:'';
 await add(`kechengzuoye`, ruleForm)
}
}
}
else {
