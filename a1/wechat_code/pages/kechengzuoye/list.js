...
res = await page(`kechengzuoye`, searchForm);
} else {
res = await list(`kechengzuoye`, searchForm);
}
...
wx.navigateTo({
url: `/pages/kechengzuoye/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("kechengzuoye",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
wx.navigateTo({
url: `/pages/kechengzuoye/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/kechengzuoye/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 kechengzuoye Kechengzuoye kechengzuoye ${pathname}
console.log('Kechengzuoye  kechengzuoye')
isAuthObj['addAuth']   = utils.isAuth("kechengzuoye", "新增")
isAuthObj['delAuth']  = utils.isAuth("kechengzuoye", "删除")
isAuthObj['editAuth']   =utils.isAuth("kechengzuoye", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("kechengzuoye", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("kechengzuoye", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("kechengzuoye", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("kechengzuoye",obj)
    }else{
         resList= await list("kechengzuoye",obj)
    }
