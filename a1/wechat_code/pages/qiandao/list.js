...
if(this.data.banji){
    searchForm['banji'] = '%' + this.data.banji + '%'
}
...
res = await page(`qiandao`, searchForm);
} else {
res = await list(`qiandao`, searchForm);
}
...
wx.navigateTo({
url: `/pages/qiandao/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("qiandao",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
wx.navigateTo({
url: `/pages/qiandao/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/qiandao/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 qiandao Qiandao qiandao ${pathname}
console.log('Qiandao  qiandao')
isAuthObj['addAuth']   = utils.isAuth("qiandao", "新增")
isAuthObj['delAuth']  = utils.isAuth("qiandao", "删除")
isAuthObj['editAuth']   =utils.isAuth("qiandao", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("qiandao", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("qiandao", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("qiandao", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("qiandao",obj)
    }else{
         resList= await list("qiandao",obj)
    }
