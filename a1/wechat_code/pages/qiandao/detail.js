...
if (id) {
const {data} = await detail("qiandao",id)
this.setData({
payAuth:utils.isAuthFront('qiandao','支付')
})
...
const resUpdate = await update('qiandao', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','qiandao');
...
async reply() {
const detailList = this.data.detailList
const res = await update("qiandao", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','qiandao');
