...
let dataEmpty={};
const  def_10= 0  ;
this.data.dengjiList.map((v, index) => {
if (v == data.dengji|| (v == def_10 && def_10 !=null)) {
 this.setData({
   dengjiIndex: index,
   dengji: v,
 })
}else if (this.data.dengjiList.length == 1) {
 this.setData({
   dengjiIndex: 0,
   dengji: v,
 })
...
if(o=='zuoyefenshu') {
dataEmpty["zuoyefenshu"] = data[o]
}
if(o=='pingjia') {
dataEmpty["pingjia"] = data[o]
}
if(o=='dengji') {
dataEmpty["dengji"] = data[o]
}
}
...
dengjiChange(e) {
this.setData({
 dengjiIndex:   e.detail.value,
 dengji :this.data.dengjiList[e.detail.value]
})
},
...
pingjiaInput(e){
this.setData({
pingjia: e.detail.value // 每次输入变化时更新文本框的值
});},
...
// 在 Page 中的某个方法中调用
query.select('#pingjia').boundingClientRect();
query.exec(function (res) {
//res就是 所有标签为v1的元素的信息 的数组
that.setData({
pingjia: res[0]?.dataset?.info
})
})
...
zuoyefenshu: this.data. zuoyefenshu,
pingjia: this.data. pingjia,
dengji:this.data.dengji?this.data.dengji:'',
}
...
const valuezuoyefenshu = this.data.zuoyefenshu;
if(isNaN(valuezuoyefenshu)  && this.data.zuoyefenshu){
wx.showToast({
icon: "none",
title: `作业分数应输入数字`,
})
...
let corssRes = await list(`zuoyepigai`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
//跨表计算
if (ruleForm.id ) {
await update(`zuoyepigai`, ruleForm)
}
else {
 this.data?.commentScore?ruleForm['score']= this.data.commentScore:'';
 await add(`zuoyepigai`, ruleForm)
}
}
}
else {
