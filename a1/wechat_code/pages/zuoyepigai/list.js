...
res = await page(`zuoyepigai`, searchForm);
} else {
res = await list(`zuoyepigai`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zuoyepigai/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zuoyepigai",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
wx.navigateTo({
url: `/pages/zuoyepigai/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/zuoyepigai/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 zuoyepigai Zuoyepigai zuoyepigai ${pathname}
console.log('Zuoyepigai  zuoyepigai')
isAuthObj['addAuth']   = utils.isAuth("zuoyepigai", "新增")
isAuthObj['delAuth']  = utils.isAuth("zuoyepigai", "删除")
isAuthObj['editAuth']   =utils.isAuth("zuoyepigai", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("zuoyepigai", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("zuoyepigai", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("zuoyepigai", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("zuoyepigai",obj)
    }else{
         resList= await list("zuoyepigai",obj)
    }
