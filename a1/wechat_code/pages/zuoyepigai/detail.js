...
if (id) {
const {data} = await detail("zuoyepigai",id)
this.setData({
payAuth:utils.isAuthFront('zuoyepigai','支付')
})
...
const resUpdate = await update('zuoyepigai', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zuoyepigai');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zuoyepigai", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','zuoyepigai');
