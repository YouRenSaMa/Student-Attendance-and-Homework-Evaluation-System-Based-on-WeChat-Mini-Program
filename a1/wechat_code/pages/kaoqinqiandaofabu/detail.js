...
const {
deleteData, update, add, page, list, detail, save,
...
qiandaoisAuthFront:'',
isAuthqiandao:'',
weidakashuomingisAuthFront:'',
isAuthweidakashuoming:'',
},
async    onLoad(option) {
let authobj={}
authobj['pageIsAuth']= wx.getStorageSync('isAuth')
authobj['isAuthqiandao']= utils.isAuth('kaoqinqiandaofabu','签到')
authobj['isAuthweidakashuoming']= utils.isAuth('kaoqinqiandaofabu','未打卡说明')
authobj['qiandaoisAuthFront']=  utils.isAuthFront('kaoqinqiandaofabu','签到')
authobj['weidakashuomingisAuthFront']=  utils.isAuthFront('kaoqinqiandaofabu','未打卡说明')
...
data['total'] =wx.getStorageSync('vip')=='是'? 0 : parseFloat(this.data.detailList.price * this.data.activeSeat.length).toFixed(2);
...
if (id) {
const {data} = await detail("kaoqinqiandaofabu",id)
this.setData({
payAuth:utils.isAuthFront('kaoqinqiandaofabu','支付')
})
...
let objClo={}
objClo['picture']= data.kechengtupian.split(',');
...
this.setData({
title:  this.data.detailList.kechengmingcheng,
    });
...
const resUpdate = await update('kaoqinqiandaofabu', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["kechengtupian"] = data.kechengtupian
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','kaoqinqiandaofabu');
...
async reply() {
const detailList = this.data.detailList
const res = await update("kaoqinqiandaofabu", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','kaoqinqiandaofabu');
