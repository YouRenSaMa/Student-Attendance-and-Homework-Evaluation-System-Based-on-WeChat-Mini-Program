...
user,
id: myid,
isAuth:utils.isAuth('weidakashuoming','审核'),
isAuthFront:  utils.isAuthFront('weidakashuoming','审核'),
...
if (id) {
const {data} = await detail("weidakashuoming",id)
this.setData({
payAuth:utils.isAuthFront('weidakashuoming','支付')
})
...
const resUpdate = await update('weidakashuoming', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','weidakashuoming');
...
const res = await update("weidakashuoming", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','weidakashuoming');
