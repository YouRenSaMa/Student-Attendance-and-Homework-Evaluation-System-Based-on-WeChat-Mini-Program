...
if(this.data.xueshengxingming){
    searchForm['xueshengxingming'] = '%' + this.data.xueshengxingming + '%'
}
...
res = await page(`weidakashuoming`, searchForm);
} else {
res = await list(`weidakashuoming`, searchForm);
}
...
item.kechengtupian = item.kechengtupian?.split(",")[0];
        return item
    })
...
wx.navigateTo({
url: `/pages/weidakashuoming/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("weidakashuoming",[id])
        console.log(res);
        if (res.code == 0) {
            this.onShow()
        }
...
editBtn(e) {
const id = e.currentTarget.dataset.id;
getApp().globalData.detailId=e.currentTarget.dataset.id;
wx.navigateTo({
url: `/pages/weidakashuoming/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
const item = e.currentTarget.dataset.item;
getApp().globalData.detailId = e.currentTarget.dataset.item.id
getApp().globalData.detailList =item
wx.navigateTo({
    url: `/pages/weidakashuoming/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 weidakashuoming Weidakashuoming weidakashuoming ${pathname}
console.log('Weidakashuoming  weidakashuoming')
isAuthObj['addAuth']   = utils.isAuth("weidakashuoming", "新增")
isAuthObj['delAuth']  = utils.isAuth("weidakashuoming", "删除")
isAuthObj['editAuth']   =utils.isAuth("weidakashuoming", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("weidakashuoming", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("weidakashuoming", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("weidakashuoming", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("weidakashuoming",obj)
    }else{
         resList= await list("weidakashuoming",obj)
    }
...
item.kechengtupian? item.kechengtupian = item.kechengtupian?.split(",")[0].replace('upload','flie'):""
return item;
});
