<template>
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
                    ...
                    <el-button class="statis_btn" type="warning" @click="echartClick1" v-if="btnAuth('tijiaozuoye','作业提交数')">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id)"  v-if="btnAuth('tijiaozuoye','删除')">
							...
							删除						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('tijiaozuoye','作业批改')" type="success" @click="zuoyepigaiCrossAddOrUpdateHandler(scope.row,'cross','','','zhuangtai','该作业已批改','已批改,待批改'.split(',')[0])">
		...
		<el-dialog v-model="echartVisible" title="统计图" width="70%">
            <div  id="tijiaozuoyeEchart1" style="width:100%;height:600px;"></div>
			<template #footer>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var tijiaozuoyeEchart1 = echarts.init(document.getElementById("tijiaozuoyeEchart1"),'theme');
			context.$http({
				url: `${tableName}/value/zuoyemingcheng/tijiaozuoye?order=desc`,
				method: 'get'
			}).then(res=>{
    ...
    const zuoyepigaiCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			zuoyepigaiFormModelRef.value.init(row.id,'cross','作业批改',row,'tijiaozuoye',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
