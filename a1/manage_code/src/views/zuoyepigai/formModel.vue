		...
		<el-dialog modal-class="edit_form_modal" class="edit_form" v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" :rules="rules">
				<el-row >
					...
					<el-col :span="12">
						<el-form-item label="作业分数" prop="zuoyefenshu">
							<el-input class="list_inp" v-model.number="form.zuoyefenshu" placeholder="作业分数"
                                type="number"
								:readonly="!isAdd||disabledForm.zuoyefenshu?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="12">
						<el-form-item label="等级" prop="dengji">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.dengji?true:false"
								v-model="form.dengji" 
								placeholder="请选择等级"
								>
								<el-option v-for="(item,index) in dengjiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="评价" prop="pingjia">
							<el-input v-model="form.pingjia" placeholder="评价" type="textarea"
							:readonly="!isAdd||disabledForm.pingjia?true:false"
							/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="提交作业内容" prop="tijiaozuoyeneirong">
							<editor :value="form.tijiaozuoyeneirong" placeholder="请输入提交作业内容" :readonly="!isAdd||disabledForm.tijiaozuoyeneirong?true:false"
								class="list_editor" @change="(e)=>editorChange(e,'tijiaozuoyeneirong')"></editor>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.tijiaozuoyeneirong = res.data.data.tijiaozuoyeneirong?(res.data.data.tijiaozuoyeneirong.replace(reg,'../../../cl526311166/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='tijiaozuoyeneirong'){
					form.value.tijiaozuoyeneirong = row[x];
					disabledForm.value.tijiaozuoyeneirong = true;
					continue;
				}
				if(x=='tijiaoshijian'){
				...
				if(x=='zuoyefenshu'){
					form.value.zuoyefenshu = row[x];
					disabledForm.value.zuoyefenshu = true;
					continue;
				}
				if(x=='pingjia'){
		...
		dengjiLists.value = "优秀,良好,中等,不合格".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
